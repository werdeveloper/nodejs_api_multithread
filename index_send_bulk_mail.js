...
const cpus = os.cpus;
const numCPUs = cpus().length; // Getting the No. of CPU
...
const forkMeBud = (result) => {
  console.log("Process Start", new Date().getTime());

  if (numCPUs > 1) {
    if (cluster.isPrimary) {
      console.log(`Primary (Parent) ${process.pid} is running`);
      ...
      for (let i = 0; i < numCPUs; i++) {
        ...
        if (result) {
          const batchSize = Math.ceil(result.length / numCPUs);
          const start = i * batchSize;
          const end = start + batchSize;
          console.log('Batch size ', start, end);
          const batchData = result.slice(start, end);
          worker.send({ data: batchData }); // Send the data to process.on('message'
        }
      }
      ...
      try {
        process.on("message", async (message) => {
          for (let i = 0; i < message.data.length; i++) {
            console.log(
              `Process on worker ${process.pid}`,
              `User Id ${message.data[i]?.id}`
            );
          }
          start();
    ...
    console.log("Single CPU is running");
